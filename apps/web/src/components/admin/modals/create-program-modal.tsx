...
export function CreateProgramModal({ isOpen, onClose, onSuccess, programToEdit }: CreateProgramModalProps) {
    ...
    // Update AI prompt when title changes
    const title = watch('title');
    useEffect(() => {
        if (title && !programToEdit) {
            setAiPrompt(title);
        }
    }, [title, programToEdit]);
    ...
    useEffect(() => {
        if (programToEdit) {
            reset({
                ...
                thumbnail: programToEdit.thumbnail || '',
            });
        } else {
            reset({
                title: '',
        ...
        >
            <div className="flex flex-col lg:flex-row gap-6">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex-1">
                    ...
                    {/* Thumbnail URL Input */}
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-platinum">Imagen de Portada (URL)</label>
                        <input
                            {...register('thumbnail')}
                            className="w-full px-3 py-2 bg-black/50 border border-white/10 rounded-lg text-white focus:outline-none focus:border-neural-blue transition"
                            placeholder="https://..."
                ...
                {/* AI Generator Column */}
                <div className="w-full lg:w-72 border-t lg:border-t-0 lg:border-l border-white/10 pt-6 lg:pt-0 lg:pl-6">
                    <AIImageGenerator
                        onImageGenerated={(url) => setValue('thumbnail', url)}
